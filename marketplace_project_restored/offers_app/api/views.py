...
from .permissions import IsOwnerOrReadOnly, IsBusinessUser, IsOwnerOnly
...
class PublicOfferListView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        valid_orderings = ['created_at', '-created_at', 'updated_at', '-updated_at', 'min_price', '-min_price']
    ...
    def _apply_filters(self, queryset):
        ...
        # KORREKT: Filter direkt auf die berechneten min_price und min_delivery_time Felder
        ...
        if min_price:
            try:
                ...
                if min_price >= 0:
                    # Filter basierend auf der Annotation calculated_min_price (>= für minimum)
                    queryset = queryset.filter(calculated_min_price__gte=min_price)
            except (ValueError, TypeError):
                # Invalid values are ignored, return all offers
                pass
        
        # Min delivery time filter - Filtert auf das min_delivery_time Feld des Angebots (<=)
        max_delivery_time = self.request.query_params.get('max_delivery_time')
        if max_delivery_time:
            try:
                max_delivery_time = int(max_delivery_time)
                if max_delivery_time > 0:
                    # Filter basierend auf der Annotation calculated_min_delivery (<= für maximum)
                    queryset = queryset.filter(calculated_min_delivery__lte=max_delivery_time)
            except (ValueError, TypeError):
                # Invalid values are ignored, return all offers
                pass
...
class MyOffersView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        return Offer.objects.filter(owner=self.request.user).select_related('owner').prefetch_related('offer_details')
