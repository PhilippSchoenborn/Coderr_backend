...
urlpatterns = [
    ...
    path('<int:pk>/', views.ProfileDetailView.as_view(), name='profile-detail'),  # GET/PATCH /api/profile/{pk}/
    
    # For /api/profiles/ endpoints  
    path('', views.ProfileView.as_view(), name='profile-root'),  # Root endpoint
    path('<int:user_id>/', views.ProfileByUserIdView.as_view(), name='profile-by-user-id'),
    path('profile/<int:pk>/', views.ProfileDetailView.as_view(), name='profile-detail-legacy'),
